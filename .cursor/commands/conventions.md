...
```python
# ❌ WRONG - Bypasses all RLS, anyone can read any data
supabase = create_client(URL, SERVICE_KEY)

# ✅ CORRECT - Uses user JWT, RLS enforced
...
```sql
-- ❌ WRONG - Race condition on concurrent joins
SELECT game_id FROM share_tokens WHERE token = $1

-- ✅ CORRECT - Locks row until transaction completes
...
```python
# ❌ WRONG - No context, polysemy issues
embedding = await get_embedding(word)

# ✅ CORRECT - Context disambiguates meaning
...
├── models.py            # Pydantic models (DO NOT MODIFY SHAPES)
├── middleware/
│   └── auth.py          # JWT validation (CRITICAL)
├── routes/
│   ├── games.py         # Game CRUD endpoints
│   ├── embeddings.py    # Noise floor, validation
│   ├── users.py         # User profile endpoints
│   └── share.py         # Share token endpoints
└── services/
    ├── embeddings.py    # OpenAI integration
    ├── scoring.py       # Divergence/convergence math
